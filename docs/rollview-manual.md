...
- The device decides when an automatic sync happens, from its **Sync options** settings, **Auto sync** and **Auto sync delay**. It holds off while a measurement is running and while the operator is on a measurement's own page, so a folder is never taken out from under them.
- Pressing **Sync data** here syncs immediately, whatever those settings are. A device showing a folder that the sync removes returns to its main screen.
...
- **Slope [g]:** Fitted straight-line slope of the mean profile using normalized roll distance from 0 to 1.
...
- **Manual re-run**: Users can manually trigger a re-run of activated postprocessors for all previous measurements (all roll folders in the current working directory) from the Postprocessors menu by selecting **Run postprocessors**. By default, this manual re-run is limited to files no older than 10 days, but this limit can be changed by modifying the setting `POSTPROCESSORS_RECENT_CUTOFF_TIME_DAYS`. To re-run them for a single roll instead, right-click that folder in the folder list and select **Run postprocessors for this folder**; the age limit does not apply there, since the folder was named explicitly.
- **Custom postprocessors**: Users can create additional postprocessors to perform custom tasks, such as automatically exporting data to mill systems. These custom postprocessors must follow the same syntax as the default ones. Custom postprocessors can be placed in a folder named `postprocessors` under `~/.tapiorqp/`. This folder is always read from there, whichever working directory is selected in settings; **Open postprocessor folder** in the Postprocessors menu opens it, creating it if it does not exist yet.
- **Loading custom postprocessors**: On software launch, the software scans the `postprocessors` folder under `~/.tapiorqp/` and loads any additional postprocessors found there. They appear in the Postprocessors menu below the default ones, separated from them by a divider, and can be activated and deactivated the same way. Modules added or edited while the software is running are picked up by **Refresh postprocessors**, so a new module does not need a restart.

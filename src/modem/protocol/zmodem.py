...
class ZMODEM(Modem):
    def __init__(self, getc, putc, thread):
    ...
    def recv(self, basedir, retry=160, timeout=60, delay=1):
        '''
    ...
    def _recv_16_data(self, timeout):
        ...
        log.debug('My CRC16 (data) = %08x, theirs = %08x' % (mine, rcrc))
        if mine != rcrc:
    ...
    def _recv_32_data(self, timeout):
        ...
        log.debug('My CRC32 (data) = %08x, theirs = %08x' % (mine, rcrc))
        if mine != rcrc:
            log.error('Invalid CRC32')
            return timeout, ''
        else:
    ...
    def _recv_hex_header(self, timeout):
        ...
        log.debug('My CRCHEX = %04x, theirs = %04x' % (mine, rcrc))
        if mine != rcrc:
    ...
    def _send_cancel_sequence(self, timeout):
        for _ in range(0, 8):
            self.putc(chr(CAN), timeout)
        for _ in range(0, 10):
            self.putc(chr(0x08), timeout)
